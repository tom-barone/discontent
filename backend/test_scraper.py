...
def test_date_range():
    ...
    assert dates[0] == datetime(2022, 12, 29)
    assert dates[1] == datetime(2022, 12, 30)
    assert dates[2] == datetime(2022, 12, 31)
    assert dates[3] == datetime(2023, 1, 1)
    assert dates[4] == datetime(2023, 1, 2)
    assert dates[5] == datetime(2023, 1, 3)
    assert dates[6] == datetime(2023, 1, 4)
    assert len(dates) == 7
...
def test_scraping():
    ...
    truth_set = [
        ...
        ('https://www.beautifulpublicdata.com/the-style-guide-for-americas-highways-mutcd/',
         198), ('https://www.starfivetech.com/en/site/new_details/976', 144),
        ...
        ('https://devblogs.microsoft.com/oldnewthing/20230109-00/?p=107685',
         230), ('https://github.com/ToolJet/ToolJet/releases/tag/v2.0.0', 210),
        ...
        ('https://findthatmeme.com/blog/2023/01/08/image-stacks-and-iphone-racks-building-an-internet-scale-meme-search-engine-Qzrz7V6T.html',
         785), ('https://store.steampowered.com/app/1261430/Kandria/', 487),
    ...
    with open('fixtures/scraper/hacker_news_front_2023_01_11.html', 'r') as f:
        html = f.read()
        soup = BeautifulSoup(html, 'html.parser')

        submissions = get_submissions(soup)
        assert len(submissions) == 30
        assert submissions == truth_set
